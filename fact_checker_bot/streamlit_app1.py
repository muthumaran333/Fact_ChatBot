...
# ================== Sidebar ==================
with st.sidebar:
    st.markdown(f"<img src='{LOGO_URL}' width='120' height='80'>", unsafe_allow_html=True)
...
def clean_text(text: str) -> str:
    """Remove bold/italic markdown and extra asterisks from text."""
    ...
    if message["bot"]:
        st.markdown(
            ...
            <div style='display: flex; align-items: flex-start; margin-bottom: 12px;'>
                ...
                <div style='background: #f4f4f4; padding: 12px 18px; border-radius: 12px;
                            max-width: 70%; color: black;'>
                    {message['bot']}
                </div>
            </div>
            """,
...
with col3:
    send_clicked = st.button("➤", help="Send")  # Custom send icon
